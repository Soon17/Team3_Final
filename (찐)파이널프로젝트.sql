...
CREATE TABLE `CAR` (
   ...
   `CR_RE_NUM`   INT   NOT NULL,
    `CR_CT_KEY` INT NOT NULL,
    `CR_THUMBNAIL` VARCHAR(100) NOT NULL
);
...
CREATE TABLE `MEMBER` (
   ...
   `ME_BIRTHDAY`   CHAR(8) NOT NULL,
    ...
    `ME_PROVIDER`   ENUM("NORMAL", "KAKAO", "NAVER", "GOOGLE") DEFAULT("NORMAL") NOT NULL,
...
CREATE TABLE `RE_RESERVATION` (
   ...
   `RR_NAME`   VARCHAR(10) NOT   NULL,
   `RR_NUMBER`   INT(15) NOT   NULL,
   `RR_BIRTH`   INT(20) NOT   NULL,
...
CREATE TABLE `LO_RESERVATION` (
   ...
   `LR_GUEST_NUMBER`   VARCHAR(20)   NULL,
...
ALTER TABLE `CAR_PACKAGE` ADD CONSTRAINT `FK_CAR_TO_CAR_PACKAGE_1` FOREIGN KEY (`CP_CR_ID`) REFERENCES `CAR` (`CR_ID`);
ALTER TABLE `CAR_PACKAGE` ADD CONSTRAINT `FK_INSURANCEL_TO_CAR_PACKAGE_1` FOREIGN KEY (`CP_IR_NUM`) REFERENCES `INSURANCEL` (`IR_NUM`);
ALTER TABLE `CAR` ADD CONSTRAINT `FK_RENTAL_TO_CAR_1` FOREIGN KEY (`CR_RE_NUM`) REFERENCES `RENTAL` (`RE_NUM`);
ALTER TABLE `CAR` ADD CONSTRAINT `FK_CAR_TYPE_TO_CAR_1` FOREIGN KEY (`CR_CT_KEY`) REFERENCES `CAR_TYPE` (`CT_KEY`);
ALTER TABLE `LODGING` ADD CONSTRAINT `FK_MEMBER_TO_LODGING_1` FOREIGN KEY (`LD_ME_NUM`) REFERENCES `MEMBER` (`ME_NUM`);
ALTER TABLE `LODGING` ADD CONSTRAINT `FK_REGION_TO_LODGING_1` FOREIGN KEY (`LD_RG_NUM`) REFERENCES `REGION` (`RG_NUM`);
ALTER TABLE `ROOM` ADD CONSTRAINT `FK_LODGING_TO_ROOM_1` FOREIGN KEY (`RM_LD_NUM`) REFERENCES `LODGING` (`LD_NUM`);
ALTER TABLE `USER_CHOICE_OPTION` ADD CONSTRAINT `FK_CHOICE_OPTION_TO_USER_CHOICE_OPTION_1` FOREIGN KEY (`UC_CO_NUM`) REFERENCES `CHOICE_OPTION` (`CO_NUM`);
ALTER TABLE `USER_CHOICE_OPTION` ADD CONSTRAINT `FK_LO_RESERVATION_TO_USER_CHOICE_OPTION_1` FOREIGN KEY (`UC_LR_NUM`) REFERENCES `LO_RESERVATION` (`LR_NUM`);
ALTER TABLE `CHOICE_OPTION` ADD CONSTRAINT `FK_DEFAULT_OPTION_TO_CHOICE_OPTION_1` FOREIGN KEY (`CO_DO_NUM`) REFERENCES `DEFAULT_OPTION` (`DO_NUM`);
ALTER TABLE `RE_RESERVATION` ADD CONSTRAINT `FK_CAR_PACKAGE_TO_RE_RESERVATION_1` FOREIGN KEY (`RR_CP_NUM`) REFERENCES `CAR_PACKAGE` (`CP_NUM`);
ALTER TABLE `RE_RESERVATION` ADD CONSTRAINT `FK_MEMBER_TO_RE_RESERVATION_1` FOREIGN KEY (`RR_ME_NUM`) REFERENCES `MEMBER` (`ME_NUM`);
ALTER TABLE `RENTAL` ADD CONSTRAINT `FK_MEMBER_TO_RENTAL_1` FOREIGN KEY (`RE_ME_NUM`) REFERENCES `MEMBER` (`ME_NUM`);
ALTER TABLE `RENTAL` ADD CONSTRAINT `FK_REGION_TO_RENTAL_1` FOREIGN KEY (`RE_RG_NUM`) REFERENCES `REGION` (`RG_NUM`);
ALTER TABLE `LO_RESERVATION` ADD CONSTRAINT `FK_MEMBER_TO_LO_RESERVATION_1` FOREIGN KEY (`LR_ME_NUM`) REFERENCES `MEMBER` (`ME_NUM`);
ALTER TABLE `LO_RESERVATION` ADD CONSTRAINT `FK_ROOM_TO_LO_RESERVATION_1` FOREIGN KEY (`LR_RM_NUM`) REFERENCES `ROOM` (`RM_NUM`);
ALTER TABLE `REVIEW` ADD CONSTRAINT `FK_MEMBER_TO_REVIEW_1` FOREIGN KEY (`RV_ME_NUM`) REFERENCES `MEMBER` (`ME_NUM`);
